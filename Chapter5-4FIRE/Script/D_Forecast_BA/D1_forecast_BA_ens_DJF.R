...
# save(mask, file = paste0(dir_data, "mask_", season, ".RData"))
rm(BA)
BA = BAS * NA
datasets = c('CPC',
             'GPCC',
...
for (idata in 1:length(datasets)) {
  ...
  spipred[, ,2:20, , 4, 3] = aux[, ,seq(last_month +12, dim(aux)[3], 12),]
  ...
  spipred[, ,2:20, , 3, 3] = aux[, , seq(last_month+11, dim(aux)[3], 12),]
  
  load(paste0(dir_s5, "SPI12SEAS5_11_DEC_",dataset,".RData"))
  spi12pred2[is.infinite(spi12pred2)] = NA
  aux = spi12pred2[, , 1:(dim(spi12pred2)[3]-12),]
  spipred[, ,1:19, , 2, 3] = aux[, , seq(first_month, (dim(aux)[3]-12), 12),]
  
  load(paste0(dir_s5, "SPI12SEAS5_11_NOV_",dataset,".RData"))
  spi12pred1[is.infinite(spi12pred1)] = NA
  aux = spi12pred1[, , 1:(dim(spi12pred1)[3]-12),]
  spipred[, ,1:19, , 1, 3] = aux[, , seq(first_month - 1, (dim(aux)[3]-12), 12),]
  ...
  for (i in 1:length(lon)) {
    for (j in 1:length(lat)) {
      ...
      if ((dataset == "CAMS_OPI") & (i == 80 & j == 42)) {
        next
      }
      
      if ((dataset == "CPC") & (i == 78 & j == 42)) {
        next
      }
      
      if ((dataset == "CPC") & (i == 80 & j == 42)) {
        next
      }
      
      if ((dataset == "CPC") & (i == 83 & j == 42)) {
        next
      }
      
      if ((dataset == "GPCC") & (i == 83 & j == 42)) {
        next
      }
      
  
      if (!is.na(best_m_spi_c_fin[i, j]) | !is.na(best_m_spi_a_fin[i, j]) & !is.na(mask_model[i, j])) {
        
        spi_a_aux  = rep(NA, length(years))
        spi_c_aux = rep(NA, length(years))
        
        if (!is.na(best_m_spi_c_fin[i, j])) {
          im_spi_c = best_m_spi_c_fin[i, j] + last_month #+ 12
          
          if (im_spi_c <= 0) {
            im_ok_spi_c = 12 + im_spi_c
            dum = spi_c[i, j, 1:(dim(spi_c)[3] - 12), best_t_spi_c_fin[i, j]]
            spi_c_aux = dum[seq(im_ok_spi_c, length(dum), 12)]
            
          } else {
            im_ok_spi_c = im_spi_c
            dum = spi_c[i, j, 13:dim(spi_c)[3], best_t_spi_c_fin[i, j]]
            spi_c_aux = dum[seq(im_ok_spi_c, length(dum), 12)]
          }
          
          spifor = array(NA, dim = c(length(years), num_ens))
          spifor = spipred[i, j, , , best_m_spi_c_fin[i, j] + 4, best_t_spi_c_fin[i, j]]
          # aux = spipred[i, j, , , best_m_spi_c_fin[i, j] + 4, 1]
          # spifor = apply(aux, c(1), mean, na.rm = T)
        ...
        if (!is.na(best_m_spi_a_fin[i, j])) {
          ...
          im_spi_a = best_m_spi_a_fin[i, j] + (first_month -12) # 0
          # print(im_spi_a)
          if (im_spi_a <= -12) {
