...
# Paleta Correlation
####################################################
brk_cor <- seq(0, 0.5, length.out = 6)
# col_cor <- c( "#6E016B","#C51B7D", "#B7E2B1", "#6AB86F",  "#00441B")
col_cor <- c( "#00441B","#6AB86F", "#B7E2B1","#C51B7D","#6E016B")
...
for (idata in 1:length(datasets)) {
  ...
  if(dataset=="DROP"){
    load(paste0(dir_out_obs, "preBApre_DROP_JJA_FDR.RData"))
  ...
  if (dataset=="DROP"){
    load(paste(dir_out_obs, "bs_drop_JJA_FDR_pval.RData", sep="")) #best_roc_drop
    best_roc_obs=bs_drop_pval
  } else {
    load(paste(dir_out_obs,"bs_obs_JJA_FDR_",dataset,"_pval.RData", sep="")) #best_roc_drop
    best_roc_obs=bs_obs_pval
  }
  
  
  best_roc_obs1=best_roc_obs
  best_roc_obs1[best_roc_obs1 > 0.5] = 0.5
  ...
  mapPlot(coastlineWorld, col="white", projection="+proj=robin",breaks = brk_cor, drawBox = FALSE, 
          main = paste('Brier Score CLIBA-',dataset,' (',season,'): 
