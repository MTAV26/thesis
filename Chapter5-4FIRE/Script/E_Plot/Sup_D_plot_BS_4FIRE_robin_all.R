...
for (idata in 1:length(datasets)) {
  ...
  if (dataset=="4FIRE"){
    load(paste(dir_out_pre, "bs_4fire_JJA_FDR_pval.RData", sep="")) #best_roc_drop
    best_roc_obs=bs_4fire_pval
  } else {
    load(paste(dir_out_pre,"bs_pred_JJA_FDR_",dataset,"_pval.RData", sep="")) #best_roc_drop
    best_roc_obs=bs_pred_pval
  }
  ...
  mapPlot(coastlineWorld, col="white", projection="+proj=robin",breaks = brk_cor, drawBox = FALSE, 
          main = paste('Brier Score CLIBA-',dataset,' (S5; ',season,'): 
