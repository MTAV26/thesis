...
for (idata in 1:length(datasets)) {
  ...
  if (dataset=="4FIRE"){
    load(paste(dir_out_pre, "roc_",dataset,"_JJA_FDR.RData", sep="")) #best_roc_drop
    best_roc_obs=roca_4fire
  } else {
    load(paste(dir_out_pre,"roc_4fire_JJA_FDR_",dataset,".RData", sep="")) #best_roc_drop
    best_roc_obs=best_roc_pred
  }
  ...
  mapPlot(coastlineWorld, col="white", projection="+proj=robin",breaks = brk_cor, drawBox = FALSE, 
          main = paste0('ROC area CLIBA-',dataset,' (S5; ',season,'): 
...
image.plot(zlim=zz,legend.only=TRUE, smallplot=c(.25,.99, .6,.9),
