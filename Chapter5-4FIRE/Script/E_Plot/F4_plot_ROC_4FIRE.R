...
if (where == 'mac') {
  ...
  # dir_out="C:/Users/Usuario/Dropbox/4FIRE/data/model_4fire_FDR/"
  dir_out1="C:/Users/Usuario/Dropbox/4FIRE/Results/model_4fire_FDR_mean1/"
...
load(paste0(dir_out1, "roc_4fire_DJF_FDR.RData"))
roca_djf=roca_4fire
rm(roca_4fire)
load(paste0(dir_out1, "roc_4fire_MAM_FDR.RData"))
roca_mam=roca_4fire
rm(roca_4fire)
load(paste0(dir_out1, "roc_4fire_JJA_FDR.RData"))
roca_jja=roca_4fire
rm(roca_4fire)
load(paste0(dir_out1, "roc_4fire_SON_FDR.RData"))
roca_son=roca_4fire
rm(roca_4fire)
...
pdf(paste(dir_pdf,"Figure3.pdf"), width=9, height=6.2)
...
load(paste0(dir_out1, "/preBApre_4fire_DJF_FDR.RData"))
PredBAobs_ens=(apply(predBA, c(1, 2, 3), mean, na.rm = TRUE))
spi3=PredBAobs_ens
...
tit <-
  paste('ROC area CLIBA 4FIRE (DJF): 
...
load(paste0(dir_out1, "/preBApre_4fire_MAM_FDR.RData"))
PredBAobs_ens=(apply(predBA, c(1, 2, 3), mean, na.rm = TRUE))
spi3=PredBAobs_ens
...
tit <-
  paste('ROC area CLIBA 4FIRE (MAM): 
Skilfull surface (', round(((sum (best_roc_obs1, na.rm = TRUE)/sum(mask_dataset, na.rm = TRUE))*100),0),'%); median (', round((median(best_roc_obs, na.rm = TRUE)),2),")", sep="")
...
load(paste0(dir_out1, "/preBApre_4fire_JJA_FDR.RData"))
PredBAobs_ens=(apply(predBA, c(1, 2, 3), mean, na.rm = TRUE))
spi3=PredBAobs_ens
...
tit <-
  paste('ROC area CLIBA 4FIRE (JJA): 
Skilfull surface (', round(((sum (best_roc_obs1, na.rm = TRUE)/sum(mask_dataset, na.rm = TRUE))*100),0),'%); median (', round((median(best_roc_obs, na.rm = TRUE)),2),")", sep="")
...
load(paste0(dir_out1, "/preBApre_4fire_SON_FDR.RData"))
PredBAobs_ens=(apply(predBA, c(1, 2, 3), mean, na.rm = TRUE))
spi3=PredBAobs_ens
...
tit <-
  paste('ROC area CLIBA 4FIRE (SON): 
Skilfull surface (', round(((sum (best_roc_obs1, na.rm = TRUE)/sum(mask_dataset, na.rm = TRUE))*100),0),'%); median (', round((median(best_roc_obs, na.rm = TRUE)),2),")", sep="")
