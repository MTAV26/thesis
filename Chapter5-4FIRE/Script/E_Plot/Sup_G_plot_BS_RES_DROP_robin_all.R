...
for (idata in 1:length(datasets)) {
  ...
  if (dataset=="DROP"){
    load(paste(dir_out_obs, "bs_drop_res_JJA_FDR_pval.RData", sep="")) #best_roc_drop
    best_roc_obs=bs_drop_res_pval
  } else {
    load(paste(dir_out_obs,"bs_obs_res_JJA_FDR_",dataset,"_pval.RData", sep="")) #best_roc_drop
    best_roc_obs=bs_obs_res_pval
  }
  ...
  mapPlot(coastlineWorld, col="white", projection="+proj=robin",breaks = brk_cor, drawBox = FALSE, 
          main = paste('Brier Score Resolution CLIBA-',dataset,' (',season,'): 
