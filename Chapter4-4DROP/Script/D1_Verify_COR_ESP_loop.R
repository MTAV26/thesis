...
for (isc in 1:length(time_scale)) {
  ...
  for (idata in 1:length(datasets)) {
    ...
    for (istart_date in 1:length(start_dates)) {
      ...
      if (dataset == "MSWEP") {
        load(file.path(dir_4drop,paste('SPI',sc,'ESP_',sprintf("%02d", start_date),  "_",  target_season, "_MSWEP.RData", sep = "")))
      } else if (dataset == "JRA55") {
        load(file.path(dir_4drop,paste('SPI',sc,'ESP_',sprintf("%02d", start_date),  "_",  target_season,"_JRA55.RData",sep = "")))
      } else if (dataset == "GPCP") {
        load(file.path(dir_4drop, paste('SPI',sc,'ESP_',sprintf("%02d", start_date),  "_",  target_season, "_GPCP.RData", sep = "")))
      } else if (dataset == "GPCC") {
        load(file.path(dir_4drop, paste('SPI',sc,'ESP_',sprintf("%02d", start_date),  "_",  target_season,"_GPCC.RData",sep = "")))
      } else if (dataset == "CAMS_OPI") {
        load(file.path(dir_4drop, paste('SPI',sc,'ESP_',sprintf("%02d", start_date),  "_",  target_season,"_CAMS_OPI.RData",sep = "")))
      } else if (dataset == "PRECL") {
        load(file.path(dir_4drop, paste('SPI',sc,'ESP_',sprintf("%02d", start_date),  "_",  target_season, "_PRECL.RData",sep = "" )))
      } else if (dataset == "CPC") {
        load(file.path(dir_4drop, paste('SPI',sc,'ESP_',sprintf("%02d", start_date),  "_",  target_season, "_CPC.RData",sep = "" )))
      } else if (dataset == "CHIRPS") {
        load(file.path(dir_4drop, paste('SPI',sc,'ESP_',sprintf("%02d", start_date),  "_",  target_season, "_CHIRPS.RData", sep = "" )))
      } else if (dataset == "ERA5") {
        load(file.path(dir_4drop, paste('SPI',sc,'ESP_',sprintf("%02d", start_date),  "_",  target_season,  "_ERA5.RData", sep = "" ) ))
      } else if (dataset == "NCEP") {
        load(file.path(dir_4drop, paste('SPI',sc,'ESP_',sprintf("%02d", start_date),  "_",  target_season, "_NCEP.RData", sep = "" )))
      } else if (dataset == "MERRA2") {
        load(file.path(dir_4drop, paste('SPI',sc,'ESP_',sprintf("%02d", start_date),  "_",  target_season, "_MERRA2.RData", sep = "" )))
      } else if (dataset == "ENS") {
        load(file.path(dir_4drop, paste("4DROP_SPI", sc,"_",sprintf("%02d", start_date),  "_",  target_season, "_ENS.RData", sep = "" )))
      } else {
      ...
      if (dataset != "ENS") {
        data = apply(spi6pred[,,mesi_8,], c(1,2,3), mean, na.rm=TRUE) # 4DROP
      } else if (dataset == "ENS") {
        data = spi6pred
      } else {
        print('dataset not known')
      ...
      postscript(
        file.path(dir_out2,paste("COR_ESP_spi",sc,"_", sprintf("%02d", start_date),  "_",  target_season,"_",dataset,"_original.eps",sep = "")),
        paper = "special",
        width = 11,
        height = 7,
        horizontal = T
      ...
      tit <-
        paste(
          'Correlation for SPI6 in ', dataset, ' (ESP)  against DROP', sep=""
        )
      ...
      save(corre2, file = file.path(dir_4drop, paste("COR_ESP_spi",sc,"_",sprintf("%02d", start_date),"_",target_season,"_",dataset,"_original.RData", sep = "") ))
      save(pvalue_adj2, file = file.path(dir_4drop, paste("pvalue_ESP_spi",sc,"_",sprintf("%02d", start_date),"_",target_season,"_",dataset,"_original.RData", sep = "") ))
      ...
      postscript(
        file.path(dir_out2,paste("COR_ESP_spi",sc,"_",sprintf("%02d", start_date),"_",target_season,"_",dataset,"_detrended.eps",sep = "")),
        paper = "special",
        width = 11,
        height = 7,
        horizontal = T
      ...
      save(corre2_det, file = file.path(dir_4drop, paste("COR_ESP_spi",sc,"_",sprintf("%02d", start_date),"_",target_season,"_",dataset,"_detrended.RData", sep = "") ))
      save(pvalue_adj2_det, file = file.path(dir_out2, paste("PVALUE_ESP_spi",sc,"_",sprintf("%02d", start_date),"_",target_season,"_",dataset,"_detrended.RData", sep = "") ))
      dev.off()
    }
  }
}
