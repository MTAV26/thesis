...
lt=c()
nb = 1000
start_dates = c( 11, 1, 2, 4, 5, 7, 8, 10)
...
for (isc in 1:length(time_scale)) {
  ...
  for (istart_date in 1:length(start_dates)) {
    ...
    if (start_date == 11) { lt= " 4"
    } else if (start_date == 2) { lt = " 4"
    } else if (start_date == 5) { lt = " 4"
    } else if (start_date == 8) { lt = " 4"
    } else if (start_date == 1) { lt = " 2"
    } else if (start_date == 4) { lt = " 2"
    } else if (start_date == 7) { lt = " 2"
    } else if (start_date == 10) { lt = " 2"
    } else {
    ...
    for (ith in 1:length(thresholds)) {
      ...
      for (idata in 1:length(reg)) {
        region = reg[idata]
        print(region)
        load(paste(dir_out, "/rocarea_ESP_", (th), "_", region, "_spi", sc,"_",sprintf("%02d", start_date),  "_",  target_season,  ".RData", sep = ""))
        roc_box[, idata] = (roc_1)
      }
      roc_box = roc_box*2-1
      ...
      for (idata in 1:length(reg)) {
        region = reg[idata]
        print(region)
        load(paste(dir_out, "/rocarea_S5_", (th), "_", region, "_spi", sc,"_",sprintf("%02d", start_date),  "_",  target_season,".RData", sep = ""))
        roc_box2[, idata] = (roc_2)	
      }
      
      roc_box2 = roc_box2 * 2-1
      ...
      setEPS()
      pdf(file.path(dir_out, paste("boxplot_roc_", (cth[ith]), "_spi", sc,"_",sprintf("%02d", start_date),  "_",  target_season,  ".pdf", sep = "")),
      ...
      boxplotperc(
        ...
        ylab="ROCCS",
        ylim = c(0, 1),
        main=paste('Forecast for ', target_season,'; Lead time:', lt, 'M', sep=""),
        cex.main = 1,
