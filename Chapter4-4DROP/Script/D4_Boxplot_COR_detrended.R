...
for (isc in 1:length(time_scale)) {
  ...
  for (me in 1:length(mes)) {
    ...
    for (idata in 1:length(datasets)) {
      ...
      load(paste(dir_out,"COR_ESP_spi", sc,"_",ms,"_",est, "_",dataset, "_detrended.RData", sep = ""))
      corre_box[, idata] = as.vector(corre2_det)
    ...
    for (idata2 in 1:length(datasets)) {
      ...
      load(paste(dir_out,"COR_S5_spi", sc,"_",ms,"_",est, "_",dataset2, "_detrended.RData", sep = ""))
      corre_box2[, idata2] = as.vector(corre2_det)
      
    }
    #load(paste(dir_out,"corre_spi_",sc,"_",dataset,"_ENS.RData",sep = ""))
    
    #corre = corre2[, which(latGPCP > -60 & latGPCP < 85)]
    ...
    postscript(
      file.path(
        dir_out,
        paste("BOX_COR_MB_DIF_spi", sc,"_",ms,"_",est,  "_detrended.eps", sep = "")
