...
for (isc in 1:length(time_scale)) {
  ...
  for (me in 1:length(mes)) {
    ...
    save(spi6pred, file = paste0(dir_out, "/4DROP_SPI", sc,"_",ms,"_",est, "_ENS_no_scaled.RData"))
    ...
    save(spi6pred, file = paste0(dir_out, "/4DROP_SPI", sc,"_",ms,"_",est, "_ENS.RData"))
    
    
    spi6pred_sd = apply(pred, c(1, 2, 3), sd, na.rm = TRUE)
    save(spi6pred_sd, file = paste0(dir_out, "/4DROP_SPI", sc,"_",ms,"_",est, "_ENS_SPREAD.RData"))
    ...
    save(spi6pred_prob, file = paste0(dir_out, "/4DROP_PROB", sc,"_",ms,"_",est, "_DROP.RData"))
    ...
    save(spi6pred_tl, file = paste0(dir_out, "/4DROP_TRAF_LIG_", sc,"_",ms,"_",est, "_DROP.RData"))
  }
}