  ...
  for (idata in 1:length(datasets)) {
    ...
    for (istart_date in 1:length(start_dates)) {
      ...
      for (ifdate in 1:length(forecast_dates)) {
        ...
        # Inicialización de matrices para las correlaciones y p-values
        corre <- matrix(data = NA,nrow = ni, ncol = nj)
        pvalue <- matrix(data = NA, nrow = ni, ncol = nj)
        
        data[is.infinite(data)]=NA
        
        # Bucle para calcular correlaciones y p-values
        for (i in 1:ni) {
          for (j in 1:nj) {
            ...
            if (n >= nt*0.9) {
              ...
              # Cálculo de correlación y p-value usando CorrMIO
        ...
        # Corrección de p-values usando método FDR y almacenamiento de resultados
...
##########################################
# para guardar todos los meses donde no hay significancia estadistica en una matriz
# Esto se usará en la aplicación de 4DROP para que el usuario pueda interpretar mejor los resultados
#############################################

# Crear una matriz para almacenar los valores significativos
significant1 <- array(NA, dim = c(dim(spi6)[1], dim(spi6)[2], 12))

# Definir nombres de meses y fechas de inicio
months <- c('01', '02', '03', '04', '05', '06', '07', '08', '09', '10', '11', '12')
start_dates <- c(2, 3, 4)

# Iterar a través de las fechas de inicio
for (start_date in start_dates) {
  
  # Iterar a través de los meses y fechas de predicción
  for (f_date in 1:12) {
    
    mesi_8 <- which(mesi == f_date)
    load(file.path(dir_out2, paste("PVALUE_ESP_spi6_", 
                                   sprintf("%02d", mesi_8[1]), "_", start_date,
                                   "M_ERA5_original.Rdata", sep = "")))
    
    # Almacenar los valores en la matriz
    significant1[, , f_date] <- pvalue_adj
    
    # Actualizar valores significativos
    significant <- significant1
    significant[significant1 <= 0.05] <- NA
    significant[significant1 >= 0.05] <- -1
    significant[is.infinite(significant)] <- NA
    significant[is.na(significant)] <- NA
    
    # Guardar el resultado en un archivo
    save(significant, file = file.path(dir_out2,
                                       paste("PVALUE_ESP_spi_all_months_",
                                       start_date, "M_ERA5_original.RData", sep = "")))
  }
}
