...
for (i in 1:12) {
  load(paste(dir_out, sprintf("BIAS_ESP_spi6_%02d_4M_ERA5_original.RData", i), sep = ""))
  corre_box[, (i * 3) - 2] = as.vector(corre)
  
  load(paste(dir_out, sprintf("BIAS_ESP_spi6_%02d_3M_ERA5_original.RData", i), sep = ""))
  corre_box[, (i * 3) - 1] = as.vector(corre)
  
  load(paste(dir_out, sprintf("BIAS_ESP_spi6_%02d_2M_ERA5_original.RData", i), sep = ""))
...
postscript(
  file.path(
    dir_out,
    paste("BOX_BIAS_DIF_spi_original_all.eps", sep = "")
