...
for (i in 1:12) {
  load(paste(dir_out, sprintf("COR_OBS_spi6_%02d_CHIRPS.RData", i), sep = ""))
  corre_box[, (i * 3) - 2] = as.vector(corre)
  
  load(paste(dir_out, sprintf("COR_OBS_spi6_%02d_EOBS.RData", i), sep = ""))
  corre_box[, (i * 3) - 1] = as.vector(corre)
  
  load(paste(dir_out, sprintf("COR_OBS_spi6_%02d_ERA5.RData", i), sep = ""))
...
setEPS()
postscript(file.path(dir_out, paste("BOX_COR_DIF_spi_all.eps", sep = "")),
           horiz = FALSE, onefile = FALSE, width = 8.5, height = 5.5)
par(mar = c(10, 4, 2, 1))

# Creación del boxplot con colores y nombres adecuados
  
  setEPS()
  postscript(file.path(dir_out,
      paste("BOX_COR_DIF_spi_all.eps", sep = "")
    ), horiz = FALSE, onefile = FALSE, width = 8.5, height = 5.5
  )
  par(mar = c(10, 4, 2, 1))
  boxplotperc(
  ...
  legend("bottomleft",inset=0,
         c("CHIRPS","E-OBS", "ERA5"), fill=c('#f7fcb9', '#addd8e', '#31a354'), horiz=T, cex=1,  col ="white")
