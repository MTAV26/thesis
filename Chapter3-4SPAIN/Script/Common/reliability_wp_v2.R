rel_wp <- function(probs,ver,bn=10,nboot=1000,cprob=0.95,rplot=FALSE) {
    ...
    for (i in 1:nboot) {
      ...
      p.avgs <- sapply(seq(bn),function(ii) mean(probst[p.bins == ii], na.rm=TRUE))
   ...
   slope.stat<-quantile(ws_slope,probs=c(.25,0.5,.75))
    ...
    if (rplot) {  
      ...
      postscript(paste('prova.ps',sep=''),width=5,height=5)
