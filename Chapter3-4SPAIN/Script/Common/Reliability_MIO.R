...
#Reliability <- function(ano_exp, ano_obs,quantile = TRUE,nbins=5,thr = c(2/3),nboot=1000,cprob=0.95) {
...
#     rel.data$p.avgs <- rel.data$p.avgs - cprob
    rel.data$cond.probs <- rel.data$cond.probs 
    rel.data$p.avgs <- rel.data$p.avgs 
  ...
  slope.stat<-quantile(ws_slope,probs=c(.05,0.5,.95),na.rm=TRUE)
     
  # Define reliability classes
    
  if (slope.stat[1]>0.5 & slope.stat[1]<1 & slope.stat[3]>1){
    ws_cat<-5
  } else if (slope.stat[1]>=0.5 & slope.stat[3]<1){
    ws_cat<-4
  } else if (slope.stat[1]>1){
    ws_cat<-4
  } else if (slope.stat[1]>0){
    ws_cat<-3
  } else if (slope.stat[1]<0 & slope.stat[3]>0){
    ws_cat<-2
  } else if (slope.stat[2]<0){
    ws_cat<-1
  } else {
    ws_cat<-6
  }
  ...
  ws_rel$slope1<-slope.stat[1]
  ws_rel$slope3<-slope.stat[3]
  return(ws_rel)
}
