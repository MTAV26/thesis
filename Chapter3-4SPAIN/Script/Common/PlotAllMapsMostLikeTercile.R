PlotAllMapsMostLikeTercile <- function(lon,lat,dataObs=NULL,data1,data2,data3,
  ...
  if(is.null(dataObs)){
    ...
    image(lon,lat,data1,axes=F,col=ColourScales(colCateg,1)$cbelow,xlab='',ylab='',breaks=breaks)
    image(lon,lat,data2,axes=F,col=ColourScales(colCateg,1)$cnormal,xlab='',ylab='',breaks=breaks,add=T)
    image(lon,lat,data3,axes=F,col=ColourScales(colCateg,1)$cabove,xlab='',ylab='',breaks=breaks,add=T)
    # if there is overplotting data, we add more probabilistic map on top.
    if(is.null(Odata1)!= T){
      image(lon,lat,Odata1,axes=F,col=ColourScales(colCateg,0.2)$cbelow,xlab='',ylab='',breaks=breaks,add=T)
      image(lon,lat,Odata2,axes=F,col=ColourScales(colCateg,0.2)$cnormal,xlab='',ylab='',breaks=breaks,add=T)
      image(lon,lat,Odata3,axes=F,col=ColourScales(colCateg,0.2)$cabove,xlab='',ylab='',breaks=breaks,add=T)
    }
    if (is.null(dataOverPlot)!= T){
      image(lon,lat,dataOverPlot,axes=F,col=rgb(colorMask[1],colorMask[2],colorMask[3]),xlab='',ylab='',add=T)
    }
  }
  else{
    # Create probabilistic map.
    image(lon,lat,data1,axes=F,col=ColourScales(colCateg,1)$cbelow_obs,xlab='',ylab='',breaks=breaks)
    image(lon,lat,data2,axes=F,col=ColourScales(colCateg,1)$cnormal_obs,xlab='',ylab='',breaks=breaks,add=T)
    image(lon,lat,data3,axes=F,col=ColourScales(colCateg,1)$cabove_obs,xlab='',ylab='',breaks=breaks,add=T)
  }
  # add world map
  ...
  if(is.null(dataObs)){
    # Adding colorbar.
    colours <- c(ColourScales(colCateg,1)$cbelow, 
                 ColourScales(colCateg,1)$cnormal, 
                 ColourScales(colCateg,1)$cabove)
    ...
    if(is.null(Odata1)!= T){
      Ocolours <- c(ColourScales(colCateg,transparency)$cbelow,
                    ColourScales(colCateg,transparency)$cnormal,
                    ColourScales(colCateg,transparency)$cabove)
  ...
  # if we are plotting observed data
    ...
    # Adding colorbar.
    colours <- c(ColourScales(colCateg,1)$cbelow_obs, 
                 ColourScales(colCateg,1)$cnormal_obs, 
                 ColourScales(colCateg,1)$cabove_obs)
    nameLab <- list (titleg[1], titleg[2], titleg[3])
