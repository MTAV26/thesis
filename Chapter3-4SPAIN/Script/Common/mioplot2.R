mioplot2<-function (var, lon, lat, varu = NULL, varv = NULL, toptitle = NULL, 
          sizetit = NULL, units = NULL, brks = NULL, cols = NULL, bar_limits = NULL, 
          triangle_ends = NULL, col_inf = NULL, col_sup = NULL, colNA = NULL, 
          color_fun = clim.palette(), square = TRUE, filled.continents = NULL, 
          coast_color = NULL, coast_width = 1, contours = NULL, brks2 = NULL, 
          contour_lwd = 0.5, contour_color = "black", contour_lty = 1, 
          contour_label_scale = 1, dots = NULL, dot_symbol = 16, dot_size = 1,
          dots2 = NULL,dot2_symbol = o, dot2_size = 1,
          arr_subsamp = floor(length(lon)/30), arr_scale = 1, arr_ref_len = 15, 
          arr_units = "m/s", arr_scale_shaft = 1, arr_scale_shaft_angle = 1, 
          axelab = TRUE, labW = FALSE, intylat = 20, intxlon = 20, 
          axes_tick_scale = 1, axes_label_scale = 1, drawleg = TRUE, 
          subsampleg = NULL, bar_extra_labels = NULL, draw_bar_ticks = TRUE, 
          draw_separators = FALSE, triangle_ends_scale = 1, bar_label_digits = 4, 
          bar_label_scale = 1, units_scale = 1, bar_tick_scale = 1, 
          bar_extra_margin = rep(0, 4), boxlim = NULL, boxcol = "purple2", 
          boxlwd = 5, margin_scale = rep(1, 4), title_scale = 1, numbfig = NULL, 
          fileout = NULL, width = 8, height = 5, size_units = "in", 
          res = 100, ...) 
{
  excludedArgs <- c("cex", "cex.axis", "cex.lab", "cex.main", 
                    "col", "din", "fig", "fin", "lab", "las", "lty", "lwd", 
                    "mai", "mar", "mgp", "new", "oma", "ps", "tck")
  #userArgs <- .FilterUserGraphicArgs(excludedArgs, ...)
  if (!is.null(fileout)) {
    deviceInfo <- .SelectDevice(fileout = fileout, width = width, 
                                height = height, units = size_units, res = res)
    saveToFile <- deviceInfo$fun
    fileout <- deviceInfo$files
  }
  if (!is.null(dots)) {
    if (!is.array(dots) || !(length(dim(dots)) %in% c(2, 
                                                      3))) {
  ...
  if (!is.null(contours)) {
    if (!is.array(contours) || !(length(dim(contours)) == 
                                 2)) {
  ...
  colorbar <- ColorBar(brks, cols, FALSE, subsampleg, bar_limits, 
                       ...
                       FALSE, extra_labels = bar_extra_labels, draw_ticks = draw_bar_ticks, 
                       draw_separators = draw_separators, triangle_ends_scale = triangle_ends_scale, 
                       label_scale = bar_label_scale, title = units, title_scale = units_scale, 
                       tick_scale = bar_tick_scale, extra_margin = bar_extra_margin, 
                       label_digits = bar_label_digits)
  brks <- colorbar$brks
  cols <- colorbar$cols
  ...
  if (!is.null(contours)) {
    if (dim(contours)[1] != dims[1] || dim(contours)[2] != 
        dims[2]) {
  ...
  if (is.null(brks2)) {
    if (is.null(contours)) {
      ...
      brks2 <- signif(seq(ll, ul, length.out = length(brks)), 
                      2)
    }
  }
  ...
  if (!is.null(boxlim)) {
    ...
    for (i in 1:length(boxlim)) {
      if (!is.numeric(boxlim[[i]]) || length(boxlim[[i]]) != 
          4) {
  ...
  if (!is.numeric(margin_scale) || length(margin_scale) != 
      4) {
  ...
  if (length(wher) > 0) {
    lon[(wher + 1):dims[1]] <- lon[(wher + 1):dims[1]] - 
      360
  }
  ...
  if (!is.null(fileout)) {
    ...
    dev.new(units = size_units, res = res, width = width, 
            height = height)
  }
  if (drawleg) {
  ...
  if (axelab) {
    ...
    if (labW) {
      ...
      letters[xpos == 180] <- " "
      letters[xpos > 180] <- "W"
      xlabs <- paste(as.character(abs(xpos2)), letters, 
                     sep = "")
    }
    else {
      xlabs <- paste(as.character(abs(xpos)), letters, 
                     sep = "")
    }
  ...
  if (drawleg) {
    layout(matrix(1:2, ncol = 1, nrow = 2), heights = c(5, 
                                                        1))
  }
  plot.new()
  par(userArgs)
  ...
  plot.window(xlim = range(lonb$x, finite = TRUE), ylim = range(latb$x, 
                                                                finite = TRUE), xaxs = "i", yaxs = "i")
  if (axelab) {
    axis(2, at = ypos, labels = ylabs, cex.axis = cex_axes_labels, 
         tcl = cex_axes_ticks, mgp = c(0, spaceticklab + 0.2, 
                                       0))
    axis(1, at = xpos, labels = xlabs, cex.axis = cex_axes_labels, 
         tcl = cex_axes_ticks, mgp = c(0, spaceticklab + cex_axes_labels/2 - 
                                         0.3, 0))
  }
  ...
  if (square) {
    image(lonb$x, latb$x, var[lonb$ix, latb$ix], col = c(col_inf_image, 
                                                         cols, col_sup_image), breaks = c(-.Machine$double.xmax, 
                                                                                          brks, .Machine$double.xmax), axes = FALSE, xlab = "", 
          ylab = "", add = TRUE)
  }
  else {
  ...
  if (!is.null(contours)) {
    contour(lonb$x, latb$x, contours[lonb$ix, latb$ix], levels = brks2, 
            method = "edge", add = TRUE, labcex = cex_axes_labels, 
            lwd = contour_lwd, lty = contour_lty, col = contour_color)
  }
  if (!is.null(dots)) {
    data_avail <- !is.na(var)
    for (counter in 1:(dim(dots)[1])) {
      points <- which(dots[counter, , ] & data_avail, arr.ind = TRUE)
      points(lon[points[, 1]], lat[points[, 2]], pch = dot_symbol[counter], 
  ...
  if (!is.null(dots2)) {
    data_avail <- !is.na(var)
    for (counter in 1:(dim(dots2)[1])) {
      points <- which(dots2[counter, , ] & data_avail, arr.ind = TRUE)
      points(lon[points[, 1]], lat[points[, 2]], pch = dot2_symbol[counter], 
             cex = dot2_size[counter] * 3/sqrt(sqrt(length(var))), 
             lwd = dot2_size[counter] * 3/sqrt(sqrt(length(var))))
    }
  }
  coast <- map(continents, interior = FALSE, wrap = TRUE, fill = filled.continents, 
               add = TRUE, plot = FALSE)
  if (filled.continents) {
    old_lwd <- par("lwd")
    par(lwd = coast_width)
    if (min(lon) >= 0) {
      ylat <- latmin:latmax
      xlon <- lonmin:lonmax
      proj <- GEOmap::setPROJ(1, LON0 = mean(xlon), LAT0 = mean(ylat), 
                              LATS = ylat, LONS = xlon)
      lakes <- which(coastmap$STROKES$col == "blue")
      coastmap$STROKES$col[which(coastmap$STROKES$col != 
                                   "blue")] <- continent_color
  ...
  if (!is.null(boxlim)) {
    ...
    for (box in boxlim) {
      ...
      if (length(box) != 4) {
      ...
      else if (box[2] < latmin || box[4] > latmax || box[1] < 
               lonmin || box[3] > lonmax) {
        stop(paste("The limits of the", counter, "st box defined in the parameter 'boxlim' are invalid."))
      }
      else if (box[1] < 0 && box[3] > 0) {
        segments(box[1], box[2], 0, box[2], col = boxcol[counter], 
                 lwd = boxlwd[counter])
        segments(0, box[2], box[3], box[2], col = boxcol[counter], 
                 lwd = boxlwd[counter])
        segments(box[1], box[4], 0, box[4], col = boxcol[counter], 
                 lwd = boxlwd[counter])
        segments(0, box[4], box[3], box[4], col = boxcol[counter], 
                 lwd = boxlwd[counter])
        ...
        rect(box[1], box[2], box[3], box[4], border = boxcol[counter], 
             col = NULL, lwd = boxlwd[counter], lty = "solid")
      }
      counter <- counter + 1
    }
  }
  if (!is.null(varu) && !is.null(varv)) {
    ...
    uaux <- lontab[sublon, sublat] + varplotu[sublon, sublat] * 
      0.5 * arr_scale
    vaux <- lattab[sublon, sublat] + varplotv[sublon, sublat] * 
      0.5 * arr_scale
    ...
    arrows(lontab[sublon, sublat], lattab[sublon, sublat], 
           uaux, vaux, angle = angleshaft, length = lenshaft)
    posarlon <- lonb$x[1] + (lonmax - lonmin) * 0.1
    posarlat <- latmin - ((latmax - latmin) + 1)/par("pin")[2] * 
      (spaceticklab + 0.2 + cex_axes_labels + 0.6 * units_scale) * 
      par("csi")
    arrows(posarlon, posarlat, posarlon + 0.5 * arr_scale * 
             arr_ref_len, posarlat, length = lenshaft, angle = angleshaft, 
           xpd = TRUE)
    xpdsave <- par("xpd")
    par(xpd = NA)
    mtext(paste(as.character(arr_ref_len), arr_units, sep = ""), 
          line = spaceticklab + 0.2 + cex_axes_labels + 1.2 * 
            units_scale, side = 1, at = posarlon + (0.5 * 
                                                      arr_scale * arr_ref_len)/2, cex = units_scale)
    par(xpd = xpdsave)
  }
  if (drawleg) {
  ...
  if (!is.null(fileout)) 
    dev.off()
  invisible(list(brks = brks, cols = cols, col_inf = col_inf, 
                 col_sup = col_sup))
}