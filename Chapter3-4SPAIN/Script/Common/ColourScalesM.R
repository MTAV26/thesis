ColourScalesM <-function(colCateg,vtransp) {
  ...
  else if (colCateg=='RedD_Orange_BlueD') {
    cabove0<-alpha(brewer.pal(9,"Blues"),vtransp)
    cabove <- cabove0[6:9]
    cabove_obs <- cabove0[9]
    cbelow0<-alpha(brewer.pal(9,"YlOrRd"),vtransp)
    cbelow <- cbelow0[5:8]
    cbelow_obs <- cbelow0[8]
    oranges0<-alpha(brewer.pal(9,"YlOrRd"),vtransp)
    cnormal <- oranges0[2:5]
