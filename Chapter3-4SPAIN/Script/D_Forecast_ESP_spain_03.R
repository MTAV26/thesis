...
for (istart_date in 1:length(start_dates)) {
  ...
  for (idata in 1:length(datasets)) {
    ...
    for (ianni in 1:length(anni)) {
      ...
      if (target_season == 'marzo' && ianni==1) {
        next
      } 
      ...
      if (target_season == 'marzo' && start_date==12) {
        anni_resampling=anni_resampling[-1]
      } else if (target_season == 'marzo' && start_date==1) {
        anni_resampling=anni_resampling
      } else if (target_season == 'marzo' && start_date==2) {
        anni_resampling=anni_resampling
      } 
      ...
      for (ires in 1:length(anni_resampling)) {
        ...
        # djfmamjjason
         if (target_season == 'marzo' && start_date == 2) {
          ...
          PRE_FOR[, , mese_start   + 1] = prec[, , mese_2 ]
          PRE_FOR[, , mese_start   + 2] = prec[, , mese_3 ]
        } else if (target_season == 'marzo' && start_date == 12) {
        ...
        if (target_season == 'marzo') {
          spi6pred[, , mese_start + 2, ires] = spitmp[, , mese_start + 2]
